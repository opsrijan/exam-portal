...
<body>
    <div class="wrapper">
        ...
        <div class="main p-3">
            ...
            <?php
                //Score
                $sql1 = "SELECT * FROM `exams` WHERE exam_id=". $_SESSION['currExam'];
                $result1 = mysqli_query($con, $sql1);
                $row = mysqli_fetch_assoc($result1);
                echo "<h2>The results of your exam '".$row['title']."' are:</h2>";

                $sql= "SELECT * FROM `evaluations` WHERE exam_id=".$_SESSION['currExam']." AND user_id=".$_SESSION['logger'];
                $result = mysqli_query($con, $sql);
                $row=mysqli_fetch_assoc($result);
                ...
                echo "/".$num;
                //Rank
                $sql2="SELECT * FROM `evaluations` WHERE exam_id=".$_SESSION['currExam']." AND score>".$row['score'];
                $res=mysqli_query($con, $sql2);
                $rank = 1+mysqli_num_rows($res);
                echo "<br>Rank=". $rank;
                $sql2="SELECT * FROM `evaluations` WHERE exam_id=".$_SESSION['currExam'];
                $res=mysqli_query($con, $sql2);
            ...
            <?php
                ...
                while ($row=mysqli_fetch_assoc($result)){
                    ...
                    $sql2 = "SELECT * FROM `student_answers` WHERE question_id=".$row['question_id']." AND user_id=".$_SESSION['logger'];
                    $res = mysqli_query($con, $sql2);
